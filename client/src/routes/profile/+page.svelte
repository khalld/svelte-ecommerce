        ...
        try {
            ...
            .then(res => {
                if (res.status == 400 || res.status == 404 ){
                    addNotification({ text: 'Something wrong happened', type: 'danger', position: 'bottom-right', removeAfter: 6000 })
                }

                if (res.status == 200){
                    addNotification({ text: 'Information update', type: 'success', position: 'bottom-right', removeAfter: 6000 })
                }
        ...
        try {
            ...
            .then(data => {
                if (data.status == 409 || data.status == 404 || data.status == 401 || data.status == 400){
                    addNotification({ text:'Something wrong happened!', type: 'error', position: 'bottom-right', removeAfter: 6000})
                } 

                if (data.status == 202){
                    addNotification({ text: 'Password changes successfully', type: 'success', position: 'bottom-right', removeAfter: 6000 })
                }
